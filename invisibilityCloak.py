...
print("""
     ...
     as it needs to capture the background (without you in it).
""")
...
while video.isOpened():
    ...
    full_mask = cv2.inRange(hsv_img, yellow_lower, yellow_upper)

    # Clean up noise from the mask
    full_mask = cv2.morphologyEx(full_mask, cv2.MORPH_OPEN, np.ones((5, 5), np.uint8))

    # Replace detected yellow areas with background
